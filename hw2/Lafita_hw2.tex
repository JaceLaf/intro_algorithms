...
There is a $root$ node from which all elements originate, and each ends with a $null$ terminator. 
...
Since the alphabet has 26 letters, there would be at most 26 comparisons, so this sorting would take $O(26)$ comparisons, 
or $O(1)$. At the end of the string, a null terminator character would be added to indicate the end of the string, 
which would be sorted sequentially first. Since there is a constant number of letters in the alphabet, 
...
From here, we must reconstruct the tree to create a sorted list of characters.
